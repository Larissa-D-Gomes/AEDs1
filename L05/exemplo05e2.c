#include <stdio.h>
#include <math.h>
/* exemplo05e2 - v0.0.1 - 31 / 08 / 2019    Author: Larissa Domingues Gomes-650525  
    Para compilar em terminal (janela de comandos):    
    Linux      : gcc -o exemplo05e2    exemplo05e2    
    Windows: gcc -o exemplo05e2.exe  exemplo05e2 
 
   Para executar em terminal (janela de comandos):   
  Linux      :  ./exemplo5e2   Windows:   exemplo05e2  */ 
   void metodo0();
   void metodo1();
   void metodo2();
   void metodo3();
...
int main(){
   ...
   do
   {
      ...
      printf("12-ler um numero x e repetir a expressao  1.0 + (i+1.0)/fatorial (i+2.0) x vezes\n");
      scanf("%i",&x);
      switch(x)
      {
         case 0:
...
void metodo12()
{
   ...
   scanf("%i", &quantidade);
   a = somae2 (quantidade);
   printf("O valor da expressao repetida %i vezes e %f\n",quantidade, a);
}
float somae2 (int x)
{
   float i = 1.0;
   int y = 0;
   int fatorial (int x);
   float valor = 1.0 + (i+1.0)/fatorial (i+2.0);
   for ( y = 0; y < x - 1;y++)
   {
      i++;
      valor = valor * (1.0 + (i+1.0)/fatorial (i+2.0));
   } 
